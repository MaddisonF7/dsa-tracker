...
def edit_own_record(table_name, record_id, user):
    ...
    try:
        cur = mysql.connection.cursor()
        cur.execute(
            f"SELECT apprentice_id FROM {table_name} WHERE id = %s", (record_id,)
        )
        result = cur.fetchone()
        cur.close()
...
def validate_input(data, table_name, is_edit=False):
    ...
    if table_name == "User":
        ...
        # Cohort validation for user table apprentices
        role = data.get("role", "").lower()
        if role == "apprentice":
            cohort = data.get("cohort", "")
            if not cohort:
                errors.append("Cohort is required.")
...
def login():
    ...
    if request.method == "POST":
        ...
        try:
            # Check MySQL database for matching email in user record
            cur = mysql.connection.cursor()
            cur.execute("SELECT * FROM User WHERE email = %s", (email,))
            user = cur.fetchone()
            cur.close()
            # If user exists and password is hashed, redirect to home page
...
def project():
    ...
    if "user" in session:
        ...
        try:
            ...
            error_message = (
                f"An error occurred while fetching project records: {str(e)}"
            )
...
def add_record(table_name):
    try:
        ...
        if route in ["/apprentices", "/managers"] and (
        ...
        columns = [
            ...
            if column[0] not in ["created_at", "modified_date"]
        ]
        cur.close()
        ...
        if request.method == "POST":
            ...
            if table_name == "User" and "password" in new_record:
                # Get plain text password
                plain_password = new_record["password"]
                # Hash password
                new_record["password"] = generate_password_hash(plain_password)
...
def edit_record(table_name, record_id):
    ...
    if "user" in session:
        ...
        try:
            ...
            if user.get("role") != "Admin" and not edit_own_record(
                table_name, record_id, user
            ):
            ...
            if request.method == "POST":
                ...
                # Only hash password if new one entered
                if "password" in updates:
                    if not updates["password"]:
                        # Keep old password if password isn't on form
                        updates["password"] = record_dict.get("password")
                    else:
                        # Hash new password
                        updates["password"] = generate_password_hash(
                            updates["password"]
                        )
    ...
    if "user" in session:
        ...
        try:
            ...
            if table_name == "User":
                ...
                cur = mysql.connection.cursor()
                cur.execute(
                    f"SELECT role FROM {table_name} WHERE id = %s", (record_id,)
                )
                result = cur.fetchone()
                role = result[0] if result else None
                # If the role is admin check for assigned apprentices
                if role == "Admin":
                    cur.execute(
                        "SELECT COUNT(*) FROM User WHERE line_manager_id = %s",
                        (record_id,),
                    )
                    apprentice_count = cur.fetchone()[0]
            ...
            if table_name == "User" and role == "Apprentice":
                ...
                cur.execute(
                    "DELETE FROM Leave_request WHERE apprentice_id = %s", (record_id,)
                )
                cur.execute(
                    "DELETE FROM Project WHERE apprentice_id = %s", (record_id,)
                )
                mysql.connection.commit()
                cur.close()
            ...
            if table_name == "User":
                if role == "Admin":
                    # Remove line managers session and refresh it
                    session.pop("line_managers", None)
                    line_managers_session()
                elif role == "Apprentice":
                    # Remove apprentices session and refresh it
                    session.pop("apprentices", None)
                    apprentices_session()
